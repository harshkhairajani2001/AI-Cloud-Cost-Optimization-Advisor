...
async function readJsonResponse(response) {
  ...
  try {
    ...
    return {
      ...
      error:
        looksLikeHtml
          ? "The upload endpoint did not return JSON. Confirm the FastAPI backend is running and that `/api/upload` is pointing to it."
          : response.status >= 500
            ? "The analysis service returned unreadable server output. Please check the backend logs and try again."
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (error instanceof TypeError) {
        setMessage(
          "Could not reach the analysis service. Confirm `npm run dev` is running and the backend is available.",
        );
      } else {
